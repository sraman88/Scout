  ...
  for (const input of inputVariants) {
    try {
      ...
      return {
        ...
        location: item.location || item.geoLocation || item.city || item.jobLocation || "",
        currentCompany: item.currentCompany || item.company || item.companyName || item.currentPosition?.company || item.experience?.[0]?.companyName || item.experiences?.[0]?.companyName || "",
        currentTitle: item.currentPosition?.title || item.jobTitle || item.experience?.[0]?.title || item.experiences?.[0]?.jobTitle || "",
        ...
        experience: item.experiences || item.experience || item.positions || [],
        education: item.educations || item.education || item.schools || [],
        skills: item.skills || [],
